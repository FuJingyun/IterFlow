...
class DMR_SetConv(torch.nn.Module):
    def __init__(self, nb_feat_in, nb_feat_out):
        ...
        super(DMR_SetConv, self).__init__()
    ...
    def forward(self, signal, graph):
        ...
        b, n, c = signal.shape
        # graph.size[0] = size_batch * nb_points
        # n_out = nb_points
        ...
        # graph.edges 为 平铺为一维 B   N  nb_neighbors
        # graph.k_neighbors = nb_neighbors 为 32
        # edge_feature: BN x nb_neighbors x c
        edge_feature = signal[graph.edges].reshape(-1, graph.k_neighbors, c) - signal.view(b * n, 1, c)
        # edge_feature.view(-1, c) :　B N nb_neighbors x c
        # graph.edge_feats: B N nb_neighbors x 3
        # signal.shape: B N nb_neighbors x (c + 3)
        signal = torch.cat((edge_feature.view(-1, c), graph.edge_feats), -1)
        # B x N x nb_neighbors x (c + 3)
        signal = signal.view(b, n_out, graph.k_neighbors, c + 3)
        # B x (c + 3) x nb_neighbors x N
