...
class FlowHead(nn.Module):
    def __init__(self, input_dim=128):
        ...
        self.conv1_dim = 64
        self.set_conv_dim = 64
        self.conv1 = nn.Conv1d(input_dim, self.conv1_dim, 1)
        self.setconv = SetConv(input_dim, self.set_conv_dim)
        self.out_conv = nn.Sequential(
            nn.Conv1d(self.conv1_dim+self.set_conv_dim, 64, 1),
            nn.ReLU(),
            nn.Conv1d(64, 3, 1)
        )
...
class UpdateBlock(nn.Module):
    ...
    def forward(self, net, inp, corr, flow, graph):
        # self.update_block(net, inp, corr, flow, graph_context)
        # templog = get_logger("update")
        # 输入的corr是64维的
        motion_features = self.motion_encoder(flow, corr)
       # templog.info(" inp, motion_features: {},{}".format(inp.shape, motion_features.shape))
        inp = torch.cat([inp, motion_features], dim=1)  # 128d
        # inp 是输入x
        # net 是GRU单元的 hidden state
