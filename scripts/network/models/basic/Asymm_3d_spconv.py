...
class ResBlock(nn.Module):
    def __init__(self, in_filters, out_filters, dropout_rate, kernel_size=(3, 3, 3), stride=1,
        ...
        if pooling:
            if height_pooling:
                self.pool = spconv.pytorch.conv.SparseConv3d(out_filters, out_filters, kernel_size=3, stride=2,
                                                padding=1, indice_key=indice_key, bias=False)
            else:
                self.pool = spconv.pytorch.conv.SparseConv3d(out_filters, out_filters, kernel_size=3, stride=(2, 2, 1),
                                                padding=1, indice_key=indice_key, bias=False)
        self.weight_initialization()
...
class UpBlock(nn.Module):
    def __init__(self, in_filters, out_filters, kernel_size=(3, 3, 3), indice_key=None, up_key=None):
        ...
        self.up_subm = spconv.pytorch.conv.SparseInverseConv3d(out_filters, out_filters, kernel_size=3, indice_key=up_key,
                                                  bias=False)
...
class Asymm_3d_spconv(nn.Module):
    def __init__(self,
        ...
        # like con1*1 , torch.Size([B, 128, 512, 180, 32])  ->  torch.Size([B, 32, 512, 180, 32])
        # like con1*1 , torch.Size([B, 4 * init_size, 512, 180, 32])  ->  torch.Size([B, fea_out_dim, 512, 180, 32])
        self.compress = spconv.pytorch.conv.SubMConv3d(4 * init_size, polar_voxel_fea_out_dim, indice_key="compress", kernel_size=1, stride=1, padding=1,
                                                bias=True)
    ...
    def forward(self, voxel_features, coors, batch_size):
        ...
        # import pdb
        # pdb.set_trace()
        ret = spconv.pytorch.conv.SparseConvTensor(voxel_features, coors, self.sparse_shape,
                                      batch_size)
        ...
        compress_polar = self.compress(up0e)

        # polar_up0e =  compress_polar.dense() # up0e.dense()

        # print("up0e")
        # print(up0e.features.shape)

        # print("polar_up0e")
        # print(polar_up0e.shape)

        # logits = self.logits(up0e)
        # y = logits.dense()
        # print("logits")
        # print(logits.features.shape)
        return compress_polar # compress_polar polar_up0e
