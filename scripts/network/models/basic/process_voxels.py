...
class Fusion_DynamicPillarFeatureNet(PillarFeatureNet):
    """
    Correct the canvas_y and canvas_x
    """
    ...
    def __init__(self,
                 ...
                 mode='max',
                 mix = False):
        super(Fusion_DynamicPillarFeatureNet,
              self).__init__(in_channels,
                             feat_channels,
                             with_distance,
                             with_cluster_center=with_cluster_center,
        ...
        # point feature to voxel feature in max&mean mode 
        self.mix = mix
        if self.mix:
            self.mean_pfn_scatter = DynamicScatter(voxel_size, point_cloud_range,
                                            average_points=True)
            self.max_pfn_scatter = DynamicScatter(voxel_size, point_cloud_range,
                                            average_points=False)
            vfn_layers = []

            for i in range(len(feat_channels) - 1):
                cur_filters = feat_channels[i + 1]
                vfn_layers.append(
                nn.Sequential(
                    nn.Linear(2*cur_filters, cur_filters, bias=False),
                    nn.BatchNorm1d(cur_filters, eps=1e-3, momentum=0.01),
                    nn.ReLU(inplace=True)))
            self.vfn_layers = nn.ModuleList(vfn_layers)
    ...
    def map_voxel_center_to_point(self, pts_coors, voxel_mean, voxel_coors):
        ...
        #            voxel_coors[:, 2] * canvas_x + voxel_coors[:, 1])
        indices = (voxel_coors[:, 0] * canvas_y * canvas_x +
                   voxel_coors[:, 1] * canvas_x + voxel_coors[:, 2])
        assert indices.long().max() < canvas_len, 'Index out of range'
        assert indices.long().min() >= 0, 'Index out of range'
        # Scatter the blob back to the canvas
        canvas[:, indices.long()] = voxel_mean.t()
        # Step 2: get voxel mean for each point
        # print("pts_coors") [  0, 113, 321],
        voxel_index = (pts_coors[:, 0] * canvas_y * canvas_x +
                       pts_coors[:, 1] * canvas_x + pts_coors[:, 2])
        assert voxel_index.long().max() < canvas_len, 'Index out of range'
        assert voxel_index.long().min() >= 0, 'Index out of range'
        center_per_point = canvas[:, voxel_index.long()].t()
        return center_per_point
    ...
    def forward(self, features, coors):
        ...
        for i, pfn in enumerate(self.pfn_layers):
            point_feats = pfn(features)
            if self.mix :
                mean_voxel_feats, voxel_coors = self.mean_pfn_scatter(point_feats, coors)
                max_voxel_feats, _ = self.max_pfn_scatter(point_feats, coors)
                voxel_feats = self.vfn_layers[i](torch.cat([max_voxel_feats, mean_voxel_feats], dim=1))
            else:
                voxel_feats, voxel_coors = self.pfn_scatter(point_feats, coors)
            if i != len(self.pfn_layers) - 1:
                # need to concat voxel feats if it is not the last pfn
...
class DynamicPillarFeatureNet_flow4D(PillarFeatureNet):
    """Pillar Feature Net using dynamic voxelization.
    The network prepares the pillar features and performs forward pass
    through PFNLayers. The main difference is that it is used for
    dynamic voxels, which contains different number of points inside a voxel
    ...
    def __init__(self,
                 ...
                 with_voxel_center=True,
                 mode='max'):
        super(DynamicPillarFeatureNet_flow4D,
              self).__init__(in_channels,
                             feat_channels,
                             with_distance,
                             with_cluster_center=with_cluster_center,
    ...
    def forward(self, features, coors):
        ...
        Returns:
            torch.Tensor: Features of pillars.
        """
        features_ls = [features] #features = (N_point, 3)
        # Find distance of x, y, and z from cluster center
        ...
        features = torch.cat(features_ls, dim=-1) #features = (N_point, 9)
        for i, pfn in enumerate(self.pfn_layers):
            point_feats = pfn(features) # point_feats = ([N_point, 32])
            voxel_feats, voxel_coors = self.pfn_scatter(point_feats, coors)
            if i != len(self.pfn_layers) - 1:
                # need to concat voxel feats if it is not the last pfn
                feat_per_point = self.map_voxel_center_to_point(
...
class my_PillarFeatureNet(nn.Module):
    """
    Correct the canvas_y and canvas_x
    """
    def __init__(self,
                 ...
                 with_voxel_center=True,
                 mode='max'):
        super(my_PillarFeatureNet, self).__init__()
        # self.in_channels has added the declared with_cluster_center&with_voxel_center dim
        if with_cluster_center:
            in_channels += 3
        if with_voxel_center:
            in_channels += 3
        ...
        self.pfn_layer = nn.Sequential(
                    nn.Linear(self.in_channels, feat_channels, bias=False),
                    # nn.Linear(self.in_channels, feat_channels),
                    nn.BatchNorm1d(feat_channels, eps=1e-3, momentum=0.01),
                    nn.ReLU(inplace=True))
    ...
    def forward(self, features, points_in_fea, coors):
        """Forward function.
        Args:
            features (torch.Tensor): Point features or raw points in shape
                (N, M, C).
        ...
        """
        # features_ls = [features]
        features_ls = [points_in_fea]
        # Find distance of x, y, and z from cluster center

        if self._with_cluster_center:
            voxel_mean, mean_coors = self.cluster_scatter(features[:, :3], coors)
            points_mean = self.map_voxel_center_to_point(
                coors, voxel_mean, mean_coors)
            # TODO: maybe also do cluster for reflectivity
            f_cluster = features[:, :3] - points_mean[:, :3]
        ...
        # Combine together feature decorations
        features = torch.cat(features_ls, dim=-1) # input+3+3

        point_feats = self.pfn_layer(features)
        voxel_feats, voxel_coors = self.pfn_scatter(point_feats, coors)
