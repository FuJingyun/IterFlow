...
class DynamicEmbedder_4D(nn.Module):
    ...
    def __init__(self, voxel_size, pseudo_image_dims, point_cloud_range,
        ...
        self.feature_net = DynamicPillarFeatureNet_flow4D(
            in_channels=5, # 3 for LiDAR, 5 for radar
            feat_channels=(feat_channels, ),
            point_cloud_range=point_cloud_range,
            voxel_size=voxel_size,
            mode='avg')
    ...
    def forward(self, input_dict) -> torch.Tensor:
        ...
        # all_voxel_feats_sp: torch.Size([50530, 16])
        all_voxel_feats_sp = torch.cat(voxel_feats_list, dim=0)
        # all_coors_batch_sp_4d: torch.Size([50530, 5])
        all_coors_batch_sp_4d = torch.cat(voxel_coors_list, dim=0)

        # sparse_tensor_4d : torch.Size([8, 16, 256, 256, 32, 2])
        # [grid_feature_size[0], grid_feature_size[1], grid_feature_size[2], num_frames]
...
class FLOPs_DynamicEmbedder_4D(nn.Module):
    ...
    def __init__(self, voxel_size, pseudo_image_dims, point_cloud_range,
        ...
        self.voxelizer = DynamicVoxelizer(voxel_size=voxel_size,
                                          point_cloud_range=point_cloud_range)
        self.feature_net = DynamicPillarFeatureNet_flow4D(
            in_channels=5, # 3 for LiDAR,5for radar
            feat_channels=(feat_channels, ),
            point_cloud_range=point_cloud_range,
            voxel_size=voxel_size,
            mode='avg')
    ...
    def forward(self, radar_pc0, radar_pc1) -> torch.Tensor:
        voxel_feats_list = []
        voxel_coors_list = []
        batch_index = 0
        # frame_keys = sorted([key for key in input_dict.keys() if key.startswith('pc_m')], reverse=True)
        ...
        time_index = 0
        pc0 = radar_pc0
        voxel_info_list = self.voxelizer(pc0)
        voxel_feats_list_batch = []
        voxel_coors_list_batch = []
        ...
        time_index = 1
        pc1 = radar_pc1
        voxel_info_list = self.voxelizer(pc1)
