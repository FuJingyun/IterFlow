...
class Cylinder_select_t0_3d_ten(nn.Module):
    def __init__(self, num_frames):
        super(Cylinder_select_t0_3d_ten, self).__init__()
        self.num_frames = num_frames
    ...
    def forward(self, sparse_4D_tensor):
        ...
        pc0_time_value = self.num_frames

        mask_pc0 = (indices_4d[:, -1] == pc0_time_value)
        
        pc0_indices = indices_4d[mask_pc0][:, :-1]  # delete the frame_id dimension
        pc0_features = features_4d[mask_pc0]
...
class Network_4D(nn.Module):
    ...
    def forward(self, sp_tensor):
        ...
        # torch.Size([8, 16, 256, 256, 32, 2])
        # cur_up_1_print = up_1.dense()
        # print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        # print("cur_up_1_print")
        # print(cur_up_1_print.shape)
...
class Point_head(nn.Module):
    ...
    def forward_single(self, voxel_feat, voxel_coords, point_feat):
        ...
        cor1 = voxel_coords[:,2].long()
        cor2 = voxel_coords[:,1].long()
        cor3 = voxel_coords[:,0].long()

        # voxel_to_point_feat = voxel_feat[:, voxel_coords[:,2], voxel_coords[:,1], voxel_coords[:,0]].T 
        voxel_to_point_feat = voxel_feat[:, cor1, cor2, cor3].T 
