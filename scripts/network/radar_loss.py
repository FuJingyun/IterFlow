...
# import cv2
from scripts.network.utils.model_utils import *
from scripts.network.utils import *
import torch.nn.functional as F
from torch.nn import Module

def mseg_label_RRV(pc1, trans, vel1, interval, vr_thres):
    gt_sf_rg = rigid_to_flow(pc1,trans)
    gt_sf_rg_proj=torch.sum(gt_sf_rg*pc1,dim=1)/(torch.norm(pc1,dim=1))
    residual=abs(vel1-gt_sf_rg_proj/interval)
    bs_residual = torch.mean(residual, dim=1).unsqueeze(1)
    # 1 denotes static, 0 denotes moving
    mseg_label = ((residual-bs_residual)<vr_thres).type(torch.float32)
    # mseg_label = ((residual-bs_residual)<vr_thres).type(torch.bool)
...
class known_EgoMotionLoss(Module):

    def __init__(self):
        super(known_EgoMotionLoss, self).__init__()

    def forward(self, pc1, pre_trans):
        """ Compute ego-motion losses
...
class StatFlowLoss(Module):
    def __init__(self):
        super(StatFlowLoss, self).__init__()
    def forward(self, pred_f, dyn_mask, pc1, gt_trans):
        ...
        loss = torch.sum((dyn_mask)*torch.norm(gt_f-pred_f,dim=1))/torch.maximum(torch.sum(dyn_mask), torch.tensor(1).cuda())
        return loss
...
class ID_ChamferLoss(Module):
    def __init__(self, truc_dist = 4.0):
        super(ID_ChamferLoss, self).__init__()

        self.TRUNCATED_DIST = truc_dist

    def square_dis(self, src, dst):
        # src [N,3]
        # dst [M,3]
        N, _ = src.shape
        M, _ = dst.shape

        dist = -2 * torch.matmul(src, dst.transpose(0,1)) # N M
    ...
    def forward(self, pc0, pc1, pred_f, pc0_id, pc1_id):
        """ Compute self-supervised losses
        Input (torch.Tensor):
            pc1: [N,3]
            pc1: [N,3]
        ...
        Return (torch.Tensor):
            total_loss: [1]
        """
        cur_dev = pc0.get_device()
        loss = torch.tensor(0.0, device=cur_dev)
        pc0_warp = pc0[:,:3] + pred_f # N,3

        unique_pc0_ids = torch.unique(pc0_id)
        unique_pc1_ids = torch.unique(pc1_id)

        count_label = 0

        for label in unique_pc0_ids:
            if label in unique_pc1_ids:
                # count_label = count_label + 1
                mask0 = (pc0_id == label)
                mask1 = (pc1_id == label)
                cur_obj_pc0 = pc0_warp[mask0]   # P 3
                cur_obj_pc1 = pc1[mask1]   # Q 3

                dist0_1 = self.square_dis(cur_obj_pc0, cur_obj_pc1) # P Q
                # dist0, _ = torch.topk(dist0_1, 1, dim = 1, largest=False, sorted=False) # P 1
                # dist1, _ = torch.topk(dist0_1, 1, dim = 0, largest=False, sorted=False) # 1 Q
                # dist0 = dist0.squeeze(1) # P
                # dist1 = dist1.squeeze(0) # Q
                dist0, _ = torch.min(dist0_1, 1)
                dist1, _ = torch.min(dist0_1, 0)

                # dist0 = torch.clamp(dist0, max=self.TRUNCATED_DIST)
                # dist1 = torch.clamp(dist1, max=self.TRUNCATED_DIST)

                trunc_mask0 = dist0 < self.TRUNCATED_DIST
                trunc_mask1 = dist1 < self.TRUNCATED_DIST
                # loss = loss + torch.mean(dist0*trunc_mask0) + torch.mean(dist1*trunc_mask1)

                if trunc_mask0.sum()>0:
                    loss = loss + torch.mean(dist0[trunc_mask0])
                    count_label = count_label + 1
                if trunc_mask1.sum()>0:
                    loss = loss + torch.mean(dist1[trunc_mask1])
                    count_label = count_label + 1

        if count_label>0:
            loss = loss/count_label
        return loss
