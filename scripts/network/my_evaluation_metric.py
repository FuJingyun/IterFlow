...
from scripts.utils.av2_eval import compute_metrics, compute_bucketed_epe, CLOSE_DISTANCE_THRESHOLD, compute_radar_metrics




def non_rigid_leaderboard(est_flow, pc0, gt_flow, is_valid, pts_ids):
    gt_is_dynamic = torch.linalg.vector_norm(gt_flow, dim=-1) >= 0.05
    mask_ = ~est_flow.isnan().any(dim=1) & ~pc0[:, :3].isnan().any(dim=1) & ~gt_flow.isnan().any(dim=1)
    ...
    # CLOSE_DISTANCE_THRESHOLD: Final = 35.0 是否在以自车为中心35m范围内
...
def evaluate_leaderboard_v3(est_flow, rigid_flow, pc0, gt_flow, is_valid, pts_ids):
    ...
    res_dict = compute_radar_metrics(
        est_flow.detach().cpu().numpy().astype(float),
        gt_flow.detach().cpu().numpy().astype(float),
        gt_is_dynamic.detach().cpu().numpy().astype(bool),
        is_valid.detach().cpu().numpy().astype(bool),
        pc0.detach().cpu().numpy().astype(float),
    )
    return res_dict
...
class new_OfficialMetrics:
    def __init__(self, class_name = "Lidar"):
    ...
    def step(self, epe_dict, bucket_dict, radar_dict):
        """
        This step function is used to store the results of **each frame**.
        """
        for key in radar_dict:
            self.radar_eval[key].append(radar_dict[key])
    ...
    def normalize(self):
        ...
        for key in self.radar_eval:
            self.radar_eval[key] = np.mean(self.radar_eval[key])
    ...
    def print(self):
        ...
        print(self.class_name + " Version 1 Metric on EPE Three-way:")
        print(tabulate(printed_data), "\n")

        printed_data = []
        for key in self.bucketed:
            printed_data.append([key, self.bucketed[key]['Static'], self.bucketed[key]['Dynamic']])
        print(self.class_name + " Version 2 Metric on Category-based:")
        print(tabulate(printed_data, headers=["Class", "Static", "Dynamic"], tablefmt='orgtbl'), "\n")

        printed_data = []
        for key in self.radar_eval:
            printed_data.append([key,self.radar_eval[key]])
        print(self.class_name + " Version 3 Metric on radar_eval:")
        print(tabulate(printed_data), "\n")


def my_evaluate_leaderboard(est_flow, rigid_flow, pc0, gt_flow, is_valid, pts_ids, dy):
    gt_is_dynamic = dy
    mask_ = ~est_flow.isnan().any(dim=1) & ~rigid_flow.isnan().any(dim=1) & ~pc0[:, :3].isnan().any(dim=1) & ~gt_flow.isnan().any(dim=1)
    mask_no_nan = mask_ & ~gt_is_dynamic.isnan() & ~is_valid.isnan() & ~pts_ids.isnan()
