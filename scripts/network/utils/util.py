...
def my_point_ray_distance(warped_pcs, pixels, camera_projection_matrix, t_camera_lidar):
    ...
    # Transform pixel to camera coordinate frame
    camera_projection_matrix = torch.from_numpy(camera_projection_matrix[:3,:3]).float().unsqueeze(0)
    cam_pcs = torch.inverse(camera_projection_matrix).cuda() @ pixels_h
    ...
    # Transform warped points from radar to camera coordinates frame
    t_camera_lidar = torch.from_numpy(t_camera_lidar).cuda().float().unsqueeze(0)
    warped_pcs = torch.cat((warped_pcs,torch.ones((batch_size,1,npoints)).cuda()),dim=1)
    warped_pcs_cam = t_camera_lidar @ warped_pcs
