...
class my_QAG(nn.Module):
    def __init__(self, radius: float, nsample: int, is_group_fea: bool = False):
        """
        :param radius: float, radius of ball
        :param nsample: int, maximum number of features to gather in the ball
        :param use_xyz:
        ...
        self.is_group_fea = is_group_fea
    ...
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[torch.Tensor]:
        ...
        if self.is_group_fea and (features is not None):       
            # (B, C, npoint, nsample)
            grouped_features = grouping_operation(features, idx)
            return idx, grouped_xyz, grouped_features
        else:
...
class my_uniquexyz(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor) -> Tuple[torch.Tensor]:
        """
        :param xyz: (B, N, 3) xyz coordinates of the features
        :param new_xyz: (B, npoint, 3) centroids
        :param features: (B, C, N) descriptors of the features
        ...
        """
        idx = ball_query(self.radius, self.nsample, xyz, new_xyz)
        idx = torch.unique(idx, dim = -1, sorted=False)
