...
class old_DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, ori_img):
        ...
        features = self._get_features(bbox_xywh, ori_img)
        # bbox_tlwh = self._xywh_to_tlwh(bbox_xywh)
        bbox_tlwh = self.xyxy_to_tlwh(bbox_xywh)
    ...
    def xyxy_to_tlwh(self, bbox_xywh):
        if isinstance(bbox_xywh, np.ndarray):
            bbox_tlwh = bbox_xywh.copy()
        elif isinstance(bbox_xywh, torch.Tensor):
            bbox_tlwh = bbox_xywh.clone()
        bbox_tlwh[:,0] = bbox_xywh[:,0]
        bbox_tlwh[:,1] = bbox_xywh[:,1]
    ...
    def _get_features(self, bbox_xywh, ori_img):
        ...
        #     im = ori_img[y1:y2,x1:x2]
        #     im_crops.append(im)
        for box_id in range(bbox_xywh.shape[0]):
            # x1,y1,x2,y2 = self._xywh_to_xyxy(bbox_xywh[box_id])
            box = bbox_xywh[box_id]
            x1 = int(box[0])
            y1 = int(box[1])
            x2 = int(box[2])
            y2 = int(box[3])
