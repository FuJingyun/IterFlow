...
import argparse
import numpy as np
from dataprocess.vod.frame import homogeneous_transformation, project_3d_to_2d
from dataprocess.preprocess.opt_utils.vod.visualization.settings import *
from dataprocess.preprocess.opt_utils.global_param import *
from dataprocess.preprocess.opt_utils.RAFT.core.raft import RAFT
from dataprocess.preprocess.opt_utils.RAFT.core.utils.flow_viz import flow_to_image
...
def my_estimate_optical_flow(img1,img2,model, height, width):


    resize_dim =(int(IMG_WIDTH), int(IMG_HEIGHT))
    # resize_dim = (int((IMG_WIDTH/hw[0])*img1.shape[1]),int((IMG_HEIGHT/hw[1])*img1.shape[0]))
    img1 = cv2.resize(img1, resize_dim)
    img2 = cv2.resize(img2, resize_dim)
    img1_torch = torch.from_numpy(img1).cuda().unsqueeze(0).transpose(1,3)
    img2_torch = torch.from_numpy(img2).cuda().unsqueeze(0).transpose(1,3)
    opt_flow = model(img1_torch, img2_torch, 12)
    np_flow = opt_flow.squeeze(0).permute(2,1,0).cpu().detach().numpy()
    # resize_dim = (int(img1.shape[1]/(IMG_WIDTH/hw[0])),int(img1.shape[0]/(IMG_HEIGHT/hw[1])))
    # resize_dim = (int(height), int(width))
    resize_dim = (int(width), int(height))
    flow = cv2.resize(np_flow, resize_dim)
...
def init_raft():
    ...
    raft_args = argparse.Namespace(model = "/home/fjy/DeFlow-master/dataprocess/preprocess/opt_utils/raft-small.pth",\
...
def new_info_from_opt_flow(radar_data, transforms, camera_projection_matrix, opt_flow):

    radar_p = np.concatenate((radar_data[:,0:3],np.ones((radar_data.shape[0],1))),axis=1)
    radar_data_t = homogeneous_transformation(radar_p, transforms)
    uvs = project_3d_to_2d(radar_data_t,camera_projection_matrix)

    radar_opt = np.zeros_like(radar_p[:,0:2])

    for ra_id in range(radar_p.shape[0]):
        first = uvs[ra_id,1]-1
        second = uvs[ra_id,0]-1
        if first >= 0 and first < opt_flow.shape[0]:
            if second >= 0 and second < opt_flow.shape[1]:
                radar_opt[ra_id] = opt_flow[first, second]
    ...
    return uvs[:,0], uvs[:,1], radar_opt


def my_info_from_opt_flow(radar_data, transforms, camera_projection_matrix, opt_flow):
